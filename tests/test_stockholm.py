...
def test_multi_step(params: dict[str, float]) -> None:
    ...
    for step in range(num_steps):
        _, statistics2 = stockholm.simulate(
            **params, num_steps=1, step=step, graph=graph2
        )
    ...
    for key, graph, statistics in [
        ("multi", graph1, statistics1),
        ("seq", graph2, statistics2),
    ]:
        assert (
            stockholm.evaluate_num_edges(graph, True)
            == statistics["num_casual_edges"][-1]
        ), f"number of casual relationships is wrong for {key}"
        assert (
            stockholm.evaluate_num_edges(graph, False)
            == statistics["num_steady_edges"][-1]
